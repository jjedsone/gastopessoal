...
router.get('/', (req, res) => {
  try {
    ...
    // Converter tags de string para array com tratamento de erro
    const formattedTransactions = transactions.map(t => {
      ...
      if (t.tags) {
        try {
          ...
          console.error('Erro ao fazer parse de tags:', error, 'Tags:', t.tags);
          tags = [];
        }
      }
      return {
  ...
  try {
    ...
    if (transaction && transaction.tags) {
      try {
        ...
        console.error('Erro ao fazer parse de tags:', error, 'Tags:', transaction.tags);
        parsedTags = [];
      }
    }
